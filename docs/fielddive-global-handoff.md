...
| Commit | Summary |
|--------|---------|
| *(pending)* | **Docs** — Record R18D2 contractor Preview customer send link prep checkpoint (§6BN.15) |
...
| **Send/Sign/Payment/PDF/public route/lifecycle** | **Public route `/p/[token]` exists (read-only)** at **`265d8f6`** (§6BN.11); **Contractor Preview review-link bridge** at **`bab25c8`** (§6BN.12); **Contractor Preview Send gate readiness UI** at **`304ed0f`** (§6BN.13); **Contractor Preview customer send link prep** at **`845e8d5`** (§6BN.15) — **Send/email delivery still disabled**; Sign, PDF, Payment, lifecycle/status mutation remain disabled |
...
| **R18D3+** | **Next (planning only)** — actual email delivery + lifecycle/event semantics; **do not** enable Resend/`proposals.status=sent`/sent events/PDF/Sign/Payment unless explicitly approved |
...
- **Next slice:** **R18D3 planning only** — actual email delivery with Resend plus explicit lifecycle/event/status semantics (delivery attempt records, email template, sender/reply-to, failure handling, mint-at-send vs reuse session URL, customer-notified semantics)
...
**Status:** **R18C2B complete** at **`e7798a7`** (**live-verified PASS** on **`rhquhnujjnzjhweypavd`**). **R18C3A complete** at **`b51383a`**. **R18C3B complete + live-verified PASS** at **`5c47854`** (§6BN.9). **R18C4A complete** at **`8523812`** (§6BN.10). **R18C4B complete** at **`265d8f6`** (§6BN.11). **R18C4C complete** at **`bab25c8`** (§6BN.12). **R18D1 complete** at **`304ed0f`** (§6BN.13). **R18D2 complete** at **`845e8d5`** (§6BN.15). **Docs checkpoint:** **pending this commit** (prior docs: **`6339cc9`** — docs: checkpoint after R18D1 contractor Preview Send gate readiness). **Next:** **plan R18D3 email delivery + lifecycle/event semantics separately** — **do not** enable Send/email delivery, Resend, `proposals.status = sent`, sent `proposal_events`, PDF, Sign/acceptance, Payment/deposit, lifecycle/customer-notified semantics, SQL/migrations, or pricing changes unless explicitly approved (§6BN.16).
...
| Send / email delivery | **Do not** wire Send/email delivery or Resend unless explicitly approved (**R18D3 planning only**) |
...
| Next slice | **R18D3 planning only** — actual email delivery with Resend plus explicit lifecycle/event/status semantics; **R18G later** signature; **R18H later** PDF; **R18I later** Payment/deposit |
...
- **Next: R18D3 planning only** — actual email delivery with Resend plus explicit lifecycle/event/status semantics. R18D3 must decide delivery attempt records, email template, sender/reply-to, failure handling, whether to mint fresh at send time, and whether/when to mark proposal/job/customer activity as sent/customer-notified
- **R18G later:** Signature/acceptance
- **R18H later:** PDF
- **R18I later:** Payment/deposit

**Mandatory read:** **§6BN** + **§6BM** before R18D3 planning.

**Stale-section warning:** **Trust order:** Header/current checkpoint → **§6BN** latest R18 notes through **R18D2** → **§6BM** / **§6BL** → **§11 override**. **Older sections may describe send-prep, customer-send link creation, or email delivery as future/unbuilt; those are stale after R18D2 where applicable. Email delivery remains unbuilt.**
...
**Verify HEAD** is **`845e8d5`** or newer (R18D2 complete); if newer, reconcile this doc.

**Latest docs checkpoint:** **pending this commit** (prior docs: **`6339cc9`** — docs: checkpoint after R18D1 contractor Preview Send gate readiness).

**Mandatory read through §6BM** (including **§6BM.13 R18B4D smoke PASS**) and **§6BN** (R18C2B live verification + **R18C3A server boundary** §6BN.7 + **R18C3B token minting** §6BN.9 + **R18C4A orchestrator/VM** §6BN.10 + **R18C4B public route/shell** §6BN.11 + **R18C4C review-link panel** §6BN.12 + **R18D1 Send gate readiness** §6BN.13 + **R18D2 customer send link prep** §6BN.15) before any next R18 slice.

**Next action (mandatory first resume step):**

1. **Plan R18D3 email delivery + lifecycle/event semantics separately** (§6BN.16) — **do not** enable Send/email delivery, Resend, `proposals.status = sent`, sent `proposal_events`, PDF, Sign/acceptance, Payment/deposit, lifecycle/customer-notified semantics, SQL/migrations, or pricing changes without explicit scope approval.
2. **R18D2 complete** (§6BN.15; code `845e8d5`) — Contractor Preview **Prepare customer link**; POST `/api/proposals/send-prep`; freeze/reuse/refreeze + mint customer-send token; open/copy session URL; **distinct from R18C4C QA review link**; **does not** email customer or mutate lifecycle/status.
3. **R18D1 complete** (§6BN.13; code `304ed0f`) — Contractor Preview Send gate readiness checklist + email draft preview; Send button disabled; message preview link = **Available after send** until prep succeeds.
...
| *(none)* | **No `/api/proposals/send` delivery route exists after R18D2** — Send gate UI + send-prep are readiness/prep only (§6BN.13–§6BN.15) |

---

## 11. FORWARD ROADMAP / NO-DRIFT NEXT STEPS

**Current checkpoint override:** Header + **§6BN** + **§6BM** (including **§6BM.13**) + **§6BL** supersede stale checkpoint lines in this section and stale **§8 CURRENT NEXT** / old **§11 body** / old **§6AL R18 row**. **Code:** **`845e8d5` — R18D2 contractor Preview customer send link prep**. **Docs:** pending this commit (prior docs: **`6339cc9`**). **R18D2 complete** (§6BN.15). **R18D1 complete** (§6BN.13). **R18C4C complete** (§6BN.12). **R18C4B complete** (§6BN.11). **R18C4A complete** (§6BN.10). **R18C3B complete + live-verified PASS** (§6BN.9). **R18C3A complete** (§6BN.7). **R18C2B live-verified PASS** (§6BN). **R18C1** (`53973f0`); **R18C2A** (`b651c7a`). **R18A complete** (§6BM). **R18B4D smoke PASS** (§6BM.13). **Audit Remediation Track complete.** **Second whole-app audit before R18 passed** (§6BL.21) — **no blockers**. **Public route `/p/[token]` exists (read-only)** — token-resolved, sent/signed-version-only; **Contractor Preview review-link bridge for QA/open/copy** (§6BN.12); **Send gate readiness UI on Preview** (§6BN.13); **Customer send link prep on Preview** (§6BN.15) — **does not enable email Send/delivery**; Sign, PDF, Payment, lifecycle/status mutation remain disabled. **Immediate next:** **plan R18D3 email delivery + lifecycle/event semantics separately** (§6BN.16) — **do not** enable Send/email delivery, Resend, `proposals.status = sent`, sent `proposal_events`, PDF/Sign/Payment/lifecycle/SQL/pricing unless explicitly approved.
