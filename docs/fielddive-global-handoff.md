...
**Phase 2 status — COMPLETE (disabled branch only):** added unwired `app/lib/proposalQuantityResolutionDisabledRawBranch.ts` plus `compareRawPlusWasteQuantityResolutionEcho` for raw↔raw echo comparison in disabled/test fixtures. Raw mode math is test-covered (source → coverage → waste → exact) but **not production-enabled**. Production adapter (`resolveProposalLineQuantityViaAdapter`) remains adjusted-only and unchanged. Adjusted quantities/echoes/totals unchanged. Staleness/preflight production chain still uses the adjusted comparer; raw compare is disabled-path only. Company policy validators and DB CHECKs still reject `raw_plus_waste`. Whole rounding still unsupported. No UI / customer/public DTO exposure. No migrations / CHECK widening.

**Phase 3 status — COMPLETE (staging + review-only migration draft):** app `validateCompanyPricingPolicy` now recognizes staged `raw_plus_waste` as a valid policy waste_model literal (pass-through; not default). `DEFAULT_WASTE_MODEL` / starter / UI lock remain `adjusted_measurement`. Whole rounding still rejected by the app validator. Pricing engine still rejects non-default waste models (no engine change). Production quantity adapter remains adjusted-only. Review-only migration draft: `supabase/migrations/20260716_023_allow_raw_plus_waste_policy_mode.sql` widens **only** `company_pricing_policies_waste_model_check` to allow `adjusted_measurement` + `raw_plus_waste`; does not widen `quantity_rounding`; does not UPDATE/backfill rows; **not applied** (REVIEW ONLY — DO NOT APPLY WITHOUT EXPLICIT APPROVAL). No UI mode control. No live company selected `raw_plus_waste`.

**Phase 4 status — COMPLETE (CHECK widening applied):** migration `supabase/migrations/20260716_023_allow_raw_plus_waste_policy_mode.sql` **live-applied + verified PASS** on approved project/ref **`rhquhnujjnzjhweypavd`**. Live `company_pricing_policies_waste_model_check` now allows `adjusted_measurement` + `raw_plus_waste`. `company_pricing_policies_quantity_rounding_check` remains `exact` only (whole blocked). Column default remains `adjusted_measurement`. Existing policy rows: **1× adjusted_measurement**, **0× raw_plus_waste** (no live company switched). No proposal/catalog schema changes; no UI mode switch; production quantity resolver remains adjusted-only; pricing engine still rejects unsupported waste models. **`raw_plus_waste` is schema-allowed but not production-active.**

**Phase 5 status — COMPLETE (backend dual-mode; smoke not run):** draft create/refresh/mapper/adapter/inspection/preflight support `raw_plus_waste` **only when** `policy.wasteModel === "raw_plus_waste"`. Default remains `adjusted_measurement` (adjusted goldens unchanged). Raw path: proven raw source (remaps `adjusted_roof_squares` → handoff `roof_squares` only) → coverage → waste → exact; persists raw `quantity_resolution_echo` with coverage/waste drivers. Pricing engine accepts raw waste model but does **not** apply coverage/waste math. Whole rounding unsupported. No UI mode switch. Customer/public DTOs still omit line echo. **Live smoke: not run** in this checkpoint (controlled plan only — do not switch the sole live company policy without explicit confirmation).

**Controlled smoke plan (manual / later):** project `rhquhnujjnzjhweypavd`; capture current `company_pricing_policies.waste_model`; temporarily set a smoke/test company (or ask before touching the single live company) to `raw_plus_waste`; draft-only create + refresh; verify raw echo round-trip + preflight `current`; restore `adjusted_measurement`. No send/public/lifecycle/UI.

**Next after Phase 5:** controlled live smoke (with restore) **or** Phase 6 read-only UI — product priority. Do not leave a live company on `raw_plus_waste` without an explicit smoke decision.
