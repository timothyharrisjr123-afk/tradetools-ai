...
- **Code checkpoint:** **`8bb19b4`** — `polish(proposals): finish contractor preview send workflow` (Preview + Send checkpoint: unified surface, elevated packet, premium Send/sharing delivery composer). Prior: **`7cb1c31`** Block 5C elevate. Builder checkpoint **unchanged** at **`05c1edb`**.
- **Docs checkpoint:** **Preview + Send workflow checkpoint** (this header + Recent committed sequence table). Prior: Block 5C elevate — sales packet (**§6BO.13.4.9** T.6.4.1).
- **Prior code:** **`7cb1c31`** Block 5C elevate sales packet; **`a275d91`** plain premium packet (not approved); **`4b9d71b`** Block 5B; **`7f1f272`** Roofr-first shell/drawer; **`05c1edb`** 4G density
- **Next coding:** **Builder** work (do **not** start in the Preview + Send pass). Do **not** add full proposal management, template rebuild/import of live options into existing drafts, supplier sync, material ordering, proposal import, CSV mapping assistant, raw mode switch, or whole rounding. **R18D3D remains blocked** until at least **Stage C4** is live and smoke-validated **plus P0 trust fixes**, then explicitly approved (§6BO.11, §6BO.13).
- **Historical note:** The Preview + Send checkpoint unifies the contractor Preview header, readiness, actions, and customer proposal into one surface; elevates the customer packet (proposal cover eyebrow, balanced Customer/Property/Project/Package snapshot, Recommended package card, framed premium estimate); and reworks Send/sharing into a premium delivery composer (branded Send-proposal header, calm readiness checkpoint, recipient card, email subject/message composer, secure-link note, blocked/ready send state, quiet delivery history) plus simplified Link/Activity tabs. Send/link/delivery APIs, readiness view-model, and unsupported (Sign/PDF/payment/QR/tracking) behavior unchanged. Builder unchanged; protected systems unchanged.
...
**Code checkpoint:** **`7f1f272`** — `polish(proposals): redesign preview proposal experience`. **Prior/interim kept: `e90d135`, `12fcd4e` (not reverted). Builder unchanged at `05c1edb`.**
...
**Status note:** T.6.2 structure approved as Roofr-first IA; **visual execution not approved** — superseded by **T.6.3** Block 5B.

**Next recommended block:** see **T.6.3**.
...
**Layout corrections vs `7f1f272`:**
- **Document width:** `CUSTOMER_PREVIEW_DOCUMENT_SHELL` full Builder-stage width (not `max-w-3xl` stacked cards)
- **Selected package:** compact summary band (no blue mega card / checkmark hero)
- **Included estimate:** flat Item / Qty / Price table from snapshot quantities; no nested scope card; no Edit/Set/Remove/Restore/Hide
- **Continuity:** one proposal surface with section dividers; readiness still compact outside document; Send/sharing still drawer closed by default

**Tests:** `proposalCustomerPreviewDocumentFirst.test.ts` Block 5B wide-document / Item·Qty·Price / package-band guards; presenter qtyLabel mapping; Builder 4G regression still pass.
