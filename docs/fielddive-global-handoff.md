...
**Next after Phase 4:** Phase 5 — draft refresh/snapshot/staleness dual-mode behind controlled test policy/smoke only. Do **not** add UI, wire production resolver as default, or switch live company policies to `raw_plus_waste` without an explicit smoke gate.
