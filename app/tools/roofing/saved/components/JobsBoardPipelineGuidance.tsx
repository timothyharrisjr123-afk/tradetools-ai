...
/**
 * Jobs Board setup chrome.
 * Healthy / loading: stay quiet (FLOW WHEN NEEDED).
 * Incomplete: actionable intervention only.
 */
export default function JobsBoardPipelineGuidance({ readiness }: JobsBoardPipelineGuidanceProps) {
  ...
  return (
    <section
      ...
      data-jobs-board-setup-action-required
    >
          ...
          return (
            <li key={step.id}>
              ...
              >
                <span
                  aria-hidden
                  className={complete ? "text-emerald-600" : unknown ? "text-slate-400" : "text-amber-600"}
                >
                  {complete ? "✓" : "○"}
                </span>
                {step.label}
