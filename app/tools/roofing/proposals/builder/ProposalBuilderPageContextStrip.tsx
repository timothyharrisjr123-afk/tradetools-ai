...
function StripButton({
  ...
  return (
    ...
    >
      {icon}
      {item.label}
      {item.showSoon ? <span className={BUILDER_PAGE_STRIP_SOON}>Soon</span> : null}
    </button>
  );
}
  ...
  return (
    <nav className={BUILDER_PAGE_STRIP} aria-label="Proposal pages">
      ...
      <button
        type="button"
        disabled
        className={`${BUILDER_PAGE_STRIP_ITEM} ${BUILDER_PAGE_STRIP_ITEM_DISABLED}`}
        title="Customer pages — navigation in a later phase"
      >
      ...
      {pageSlots.map((item, index) => (
        <span key={item.id} className="inline-flex shrink-0 items-center">
          {index === 0 ? <span className={BUILDER_PAGE_STRIP_DIVIDER} aria-hidden /> : null}
          <StripButton
            item={item}
      ...
      <span className="ml-auto inline-flex shrink-0 items-center">
        {preview ? (
          <StripButton
            item={preview}
            isActive={activePageContextId === preview.id}
            onSelect={onSelectPageContext}
          />
        ) : null}
      </span>
